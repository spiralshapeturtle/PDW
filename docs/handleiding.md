...
9. [Filters](#9-filters)
   ...
   - 9.3 [Zoeken tijdens het typen](#93-zoeken-tijdens-het-typen)
...
3. Klik op **Ontdekken...** om de chat_id terug te lezen via `getUpdates`, of plak numerieke chat_id's handmatig. 1:1-chats zijn positief, groepen negatief, supergroepen beginnen met `-100`. Scheid meerdere chat_id's met `;`.
...
| Thread-ID | Optionele `message_thread_id` voor supergroep-onderwerpen (0 = geen) |
...
Een lege belichaming valt terug op de ruwe paginatekst (Pushover vereist een niet-leeg bericht). Let op: Pushover geeft `<b>`/`<i>`/etc. niet weer tenzij **HTML-opmaak** aan staat, en het bericht is hard beperkt tot 1024 tekens. In tegenstelling tot Telegram volgt Pushover HTML-regels in HTML-modus waarbij een kale nieuwe regel slechts witruimte is; PDW zet daarom `\n` regelafbrekingen automatisch om naar `<br>` wanneer HTML-opmaak aan staat, zodat een template zoals `<b>{message}</b>\n{label}` de vetgedrukte tekst toont en elk label op zijn eigen regel. (In gewone modus behoudt Pushover nieuwe regels zoals ze zijn, maar dan worden de `<b>`-tags letterlijk getoond - zet HTML aan voor vet.) **Test** geeft een voorbeeldpagina weer via de huidige Titel/Belichamingsvelden (en het HTML-selectievakje) zodat u een voorbeeld van de echte opmaak ziet, net als Telegram.
