...
- **Trend sparkline** — the score over the last 1, 5, 15 or 60 minutes, or over the last 4 or 8 hours to review the past night, drawn in status colours: a healthy period is a solid green line, degraded stretches show orange, and time below the mail-alert threshold shows red. A thin dotted line marks the mail-alert threshold itself, so you can see how much headroom the score has before the alert would fire. The graph fills from the right (newest second at the right edge), so shortly after startup only the right-hand part contains a line yet. On the long (4/8-hour) windows one pixel covers several minutes, so besides the average trend line the sparkline draws a bold coloured band down to the *worst* value in each pixel: a brief reception drop still shows as a downward spike instead of being averaged away. Hover the sparkline to read the lowest value in the window and when it happened, e.g. `lowest 40% at 03:15`.
- **COM dot** — only when serial input is enabled: green = port open and receiving, orange ring = open but no data coming in, red = port not open.
- **Feed dots** — one dot + tag per **enabled** output feed (`SM` SMTP, `WH` webhook, `TG` Telegram, `PO` Pushover, `MQ` MQTT, `MY` MySQL, `SQ` SQLite, `TS` telnet server). Green = no known problem — this includes "enabled but nothing delivered yet", so the per-message feeds (webhook, Telegram, Pushover, SMTP) are green from startup even if their first push is days away. Orange = retrying after a temporary problem, red = the last delivery or connection failed. For colour-blind readability the retrying state also differs in shape: it draws as a hollow **ring**, while healthy and failed are both plain solid discs (green and red). Disabled feeds are not shown. The connection-oriented feeds (MQTT, MySQL, telnet server) track their connection live: red while the broker/server is unreachable, green again as soon as the connection is restored.
...
| Trend window | Sparkline window: 1 / 5 / 15 / 60 minutes, or 4 / 8 hours |
| Hide/Show health panel | Toggle the panel (right-click the same spot to bring it back) |
| Show RX needle alongside | Keep the panel but also bring the classic RX signal-strength needle back into its old far-right corner slot |

The selected health source also feeds the **RX Quality Alert** e-mail (see section 10.7), so switching the source switches what the alert reacts to. With the Penalty-system source the score falls to 0 % when nothing decodable has been received for about two minutes (dead air), so a receiver that goes silent shows red instead of holding its last healthy value; on networks with legitimate multi-minute gaps between transmissions this source therefore dips between transmissions, and the classic needle source is the better choice there. The **Show RX needle alongside** option is a third layout between "panel only" and "classic corner only": the health panel stays, but the classic RX signal-strength needle is drawn again in its old far-right corner slot. The panel shrinks just enough to make room, so the RX-health percentage stays where it is and the needle sits to its right; the old RX-Q percentage box and warning square remain hidden, since the panel's own score replaces them. Turning it on while the panel is hidden also shows the panel. On narrow windows the panel first drops its labels, then the sparkline; if even that does not fit, the classic corner is shown until there is room again. Settings are stored in `pdw.ini` under `[HealthPanel]`.
...
The MQTT feed keeps one connection open to the broker and holds it alive with keepalive (45 s), rather than reconnecting per message, so it rides out brief broker hiccups (for example a Home Assistant add-on reload or backup window) the same way any long-lived MQTT client does. If the broker drops the connection while PDW is idle (broker restart or update), the feed re-establishes it automatically within at most a minute, so the next message finds a warm connection again; a broker that stays down is retried once per minute without flooding the log. If a publish does find the connection dropped, it reconnects and retries up to four times with an exponential back-off before giving up. For unattended 24/7 use, enable **Log to file** (`MqttLogToFile=1`): a healthy quiet period shows plain `SENT` lines, and a routine reconnect shows a single `RECONNECT` followed by `SENT`. If you monitor the feed for inactivity from another system, keep that timeout well above your heartbeat interval (allow for at least two missed messages) so a single delayed publish never trips a false alarm.
