...
### v4.0.6 (July 2026)
Finishes the top-right corner display glitch first addressed in 4.0.5. No decoder-output or configuration-format changes; existing `pdw.ini` and `filters.ini` files work unchanged. See `RELEASE_NOTES.md` for the full list; highlights:
- **Divider line left of the meter (FIX [RxqSquareBandClamp], [RxqSquareDividerClip])** — companion fix to 4.0.5's [SigindDividerClip]: the RX-quality indicator square sat at a fixed vertical offset and, on the shorter themed toolbar at high DPI, poked below the divider and erased a short segment just left of the meter (re-created on every repaint, so no re-sync could heal it). The square and warning icon are now clamped above the divider and the segment under them is re-asserted on every repaint.
- **Title bar re-syncs after Remote Desktop / theme change (FIX [ToolbarResync])** — main fix for a full-width black band under the toolbar + broken divider by the signal meter, seen after the machine ran a while, often over a Remote Desktop session (e.g. Retina client). A theme/display change (which an RDP connect/disconnect triggers) silently resizes the toolbar, but PDW only recomputed the toolbar height on a window resize - so the header/divider/meter sat several pixels below the toolbar's new bottom (a black gap) until a manual resize. PDW now detects the change and re-aligns the layout automatically within ~1 second.
- **Faster re-sync after RDP / theme change (FIX [ToolbarResyncProactive])** — PDW now also reacts directly to the theme/display/DPI/setting/colour events that cause the resize, re-aligning the layout ~150 ms after the toolbar settles instead of waiting for the once-per-second check (which stays as a fallback). The meter, divider and header snap back almost immediately on connect/disconnect.
- **Title-bar header self-heals (FIX [TitleBarSelfHeal])** — safety net: the whole header row under the toolbar is refreshed once per second (previously only the RX-quality corner), so a momentarily blanked header repairs itself within a second.
- **Signal-meter/RX-Q bitmaps reload after a display change (FIX [DisplayBitmapReload])** — hygiene fix found while diagnosing the above: the meter and warning-icon bitmaps are now recreated for the new display after a display-driver change (e.g. an RDP connect/disconnect), instead of only at startup.
- **Window close no longer runs unrelated re-sync code (FIX [WmCloseFallthrough])** — internal correctness fix, no user-visible behaviour change: closing the main window shared a code path with the toolbar re-sync handlers above, so every ordinary close briefly ran that handler's code before exiting (harmless in practice, but fragile). Window close now returns directly instead of falling through.
