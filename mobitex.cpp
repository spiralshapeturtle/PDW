...
void MOBITEX::barfrog()
{
	...
	if ((i == crcc) || (nErrors < MAX_ERR))
	{
		if (bPrimaryBlock)
		{
			...
			{
				// FIX [M8]: sprintf van unsigned int in szDestination[10] kan tot 11 chars schrijven
				//           (max "4294967295\0") → 1-byte overflow. _snprintf_s met _TRUNCATE
				//           kapt netjes af binnen buffergrootte.
		...
		switch (LINK_CONTROL_INFO.FrameID)	// Handle the different frame types
		{
			...
			else if (block[3] == 15)
			{
				if ((block[4] & 0x1F) == 24)
				{
					if (block[6] == 1)
					{
						sprintf(mb_msg_buf, "TxPower=%idB RssiProcedure=%s RssiPeriod=%ims TimeToNext=%i MaxRepetitionFactor=%i�BaseStatus=%i ScanTime=%.1fsec BadBase=%idB�V GoodBase=%idB�V BetterBase=%idB�V",
											block[7],		// TX-POWER
			...
			{
				// FIX [M8]: szSender[10] = " " + BaseID[8 incl null, dus 7 chars data] + "  " = 11 bytes → 1-byte overflow.
				//           Ook szType[10] kon teveel schrijven bij groot SWEEP.type. Beide veilig truncaten.
		...
		else // this is a ROSI frame
		{
			...
			switch (block1200[7]&0x1f)
			{
				case 0x01:	// MRM  Paketdata
					if (!frametag)
						frametag = "MRM";
				case 0x02:	// ACK  Bekr�ftelse
					if (!frametag)
						frametag = "ACK";
				case 0x03:	// NACK Negativ Bekr�ftelse
					if (!frametag)
						frametag = "NACK";
				case 0x04:	// REB  Beg�rand oms�ndning
					if (!frametag)
						frametag = "REB";
				case 0x05:	// RES  Oms�ndning
					if (!frametag)
						frametag = "RES";
				case 0x06:	// ABD  Accessbeg�ran Data
					if (!frametag)
						frametag = "ABD";
				case 0x07:	// ABT  Accessbeg�ran Tal
					if (!frametag)
						frametag = "ABT";
				case 0x08:	// ABL  Accessbeg�ran Larm
					if (!frametag)
						frametag = "ABL";
				case 0x09:	// ATD  Accesstillst�nd Data
					if (!frametag)
						frametag = "ATD";
				case 0x0a:	// ATT  Accesstillst�nd Tal
					if (!frametag)
						frametag = "ATT";
				case 0x0b:	// ATL  Accesstillst�nd Larm  
					if (!frametag)
						frametag = "ATL";
				...
				case 0x10:	// TST  Tyst
					if (!frametag)
						frametag = "TST";
				case 0x11:	// AKT  Aktivitetsfr�ga
					if (!frametag)
						frametag = "AKT";
				case 0x12:	// NAT  Negativt Accesstillst�nd Tal
					if (!frametag)
						frametag = "NAT";
